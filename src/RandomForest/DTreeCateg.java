...
public class DTreeCateg {
	 ...
	 * 
	 * @param data
	 *            The data matrix as a List of int arrays - each array is one
	 *            record, each index in the array is one attribute, and the last
	 *            index is the class (ie [ x1, x2, . . ., xM, Y ]).
	 * @param forest
	...
	private void BootStrapSample(ArrayList<ArrayList<String>> data, ArrayList<ArrayList<String>> train,
		...
		for (int i = 0; i < N; i++)
			if (!in.get(i))
				test.add(data.get(i));// System.out.println("created
										// testing-data for tree :
										// "+numb);//everywhere its set to false
										// we get those to test data
	...
	private class TreeNode implements Cloneable {
		...
		public String splitValue;// check this if it return false on
									// splitonCateg
	 ...
	 * 
	 */
	public ArrayList<String> CalculateClasses(ArrayList<ArrayList<String>> traindata,
			ArrayList<ArrayList<String>> testdata, int treenumber) {
		...
		while (true) {
			if (evalNode.isLeaf)
				...
				if (evalNode.spiltonCateg) {
					...
					// if(nodeLable.equalsIgnoreCase(child.label)){//what if the
					// category is not present at all
					// evalNode = child;System.out.println("going in child
					// :"+evalNode.label);
					// found = true;
					// break;
					// }
					...
					// nodeLable =
					// changeNodeLabel(evalNode.splitAttributeM,record,evalNode.data);
	 ...
	 * number is also recorded. The {@link RandomForestWekaTest#Ms Ms}
	 * attributes are now chosen by the {@link #GetVarsToInclude()
	 * GetVarsToInclude} function</li>
		...
		if (!parent.isLeaf) {
			...
			for (int m : vars) {// m is from 0-M
				SortAtAttribute(parent.data, m);// sorts on a particular column
												// in the row
				ArrayList<Integer> DataPointToCheck = new ArrayList<Integer>();// which
																				// data
																				// points
																				// to
																				// be
			...
			// System.out.println("Adding "+parent.ChildNode.size()+" children
			// at level: "+parent.generation);
			// -------------------------------Step D
			if (!parent.spiltonCateg) {
				...
				if (!parent.left.isLeaf) {
					// System.out.println(" Splitting again from
					// "+parent.label+" at level: "+parent.generation);
				...
				for (TreeNode Child : parent.ChildNode) {
					if (!Child.isLeaf) {
						System.out.println(" Splitting again from " + parent.label + ", with data size "
								+ Child.data.size() + " at gen " + parent.generation);
	 ...
	 * sub-data matrix. Then the entropy is {@link #CalcEntropy(double[])
	 * calculated}. The lower sub-entropy and upper sub-entropy are then weight
	 * averaged to obtain the total entropy.
	 * 
	...
	private double CheckPosition(int m, int n, int Nsub, DoubleWrap lowestE, TreeNode parent, int nTre) {
		...
		if (isAlphaNumeric(real_OR_categ)) {
			...
			if (entropy < lowestE.d) {
				...
				} // System.out.println("Added "+parent.ChildNode.size()+"
	...
	private class AttributeComparatorCateg implements Comparator<ArrayList<String>> {
		 ...
		 */
		@Override
		public int compare(ArrayList<String> arg1, ArrayList<String> arg2) {// compare
																			// strings
			// TODO Auto-generated method stub
			String a = arg1.get(m);
			String b = arg2.get(m);
	...
	/**
	 * Of the M attributes, select {@link RandomForestWekaTest#Ms Ms} at random.
	 * 
