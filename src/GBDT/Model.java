...
public class Model {
	...
	public static void main(String[] args) throws IOException, Exception {
		...
		String outputModelPath = "D:/DataForMining/taolu/Lotmodel.json";
		String trainDataSet = "D:/DataForMining/taolu/taolu2rlabel.train";
		String testDataSet = "D:/DataForMining/taolu/taolu2rlabel10.test";
		String resultOutSet = "D:/DataForMining/taolu/testDatasetResult.csv";
		ArrayList<Instance> datasetTrain = Instance.loadDatasetAndSchema(trainDataSet);
		ArrayList<Instance> datasetTest = Instance.loadDatasetAndSchema(testDataSet);
		//训练集，树深度，树个数
		ArrayList<TreeNode> model = train(datasetTrain, 6, 10);
		saveModel(model, outputModelPath);
		evaluate(outputModelPath, datasetTest, resultOutSet);
	}

	private static void evaluate(String modelPath, ArrayList<Instance> dataset, String outputPath) throws Exception {
		double[] trueValues = Instance.getTrueValues(dataset);
		double[] probs = new double[dataset.size()];
		double[] trueValueTemp = new double[dataset.size()];
		for (int j = 0; j < dataset.size(); j++) {
			probs[j] = predictOneInstance(dataset.get(j), loadModel(modelPath));
			trueValueTemp[j] = dataset.get(j).trueValue;
			dataset.get(j).trueValue = probs[j];
		}
		Instance.saveDataset(dataset, trueValueTemp, outputPath);
	...
	public static ArrayList<TreeNode> loadModel(String file) throws Exception {
		...
		for (int i = 1; i < al.size() - 1; i++)
			trees.add((TreeNode) U.jsonStr2object(cutStrTail(al.get(i), 1), TreeNode.class));
		trees.add((TreeNode) U.jsonStr2object(cutStrTail(al.get(al.size() - 1), 2), TreeNode.class));
	...
	public static double[] predict(ArrayList<TreeNode> model, ArrayList<Instance> data) {
	...
	public static void saveModel(ArrayList<TreeNode> trees, String string) throws IOException, Exception {
	...
	public static double predictOneInstance(Instance s, ArrayList<TreeNode> trees) {
	...
	private static TreeNode getOneRegressionTree(int depth, final ArrayList<Instance> data)
		...
		for (int i = 0; i < threadNum; i++) {
			threads[i] = new OptimalSplitThread((ArrayList<Instance>) data.clone(), i, result, latch);
		...
		TreeNode leftNode = getOneRegressionTree(depth + 1,
				cloneSubListAndAddValue(sortedData, 0, splitIndex, bestFeatureIndex, bestRes.trueDelta));
		TreeNode rightNode = getOneRegressionTree(depth + 1, cloneSubListAndAddValue(sortedData, splitIndex,
	...
	private static ArrayList<Instance> cloneSubListAndAddValue(ArrayList<Instance> sortedData, int begin, int end,
	...
	static class OptimalSplitThread extends Thread {
		...
		public OptimalSplitThread(final ArrayList<Instance> data, int i, TreeNode[] result, CountDownLatch latch) {
			this.data = data;
			this.i = i;
			this.result = result;
			this.latch = latch;
	...
	private static TreeNode getOptimalSplit(ArrayList<Instance> data, int index) throws IOException {
		...
		for (int i = 1; i < indexs.size(); i++) {
			...
			currentRes.falseDelta = loss.getIncrement(data, endIndex, data.size());
			currentRes.loss = loss.loss(data, 0, currentRes.splitIndex, currentRes.trueDelta)
					+ loss.loss(data, currentRes.splitIndex, data.size(), currentRes.falseDelta);
			currentRes.loss += loss.penalty(currentRes.trueDelta, currentRes.falseDelta, data.size());
			loss.get(data, currentRes.trueDelta, currentRes.falseDelta, currentRes.splitIndex);
	...
	static ArrayList<Integer> getDifferentValueIndex(ArrayList<Instance> data, int index) {
	...
	public static ArrayList<TreeNode> train(ArrayList<Instance> data, int treeDepth, int treeNum) throws Exception {
