 ...
 * parameter (and fills it with random values) and outputs the result to stdout.
 * It's either done via the Add filter (second option "filter") or manual with
 * Java (second option "java").
 * <p/>
 *
...
public class AddAttribute {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < newData.numInstances(); i++) {
			// 1. nominal
			newData.instance(i).setValue(newData.numAttributes() - 2, rand.nextInt(4)); // index
																						// of
																						// labels
																						// A:0,B:1,C:2,D:3
			// 2. numeric
