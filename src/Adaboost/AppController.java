...
public class AppController {
	...
	public AppController(final DataController dataController, final Logger logger,
			final DecisionTreeClassifierHelper decisionTreeClassifierHelper, final PercentageFormat percentageFormat) {
	...
	private void computeClassifierErrors(List<Instance> testSet,
			Map<Classifier, ClassifierStatistics> weightedClassifiers) {
	...
	private void computeAdaboostTestError(List<Instance> testSet, Adaboost adaboost) {
		...
		for (Instance testInstance : testSet) {
			...
			System.out.println(predictedClass + "|" + testInstance.getClazz() + "|" + testInstance.getFeatures());
		}
